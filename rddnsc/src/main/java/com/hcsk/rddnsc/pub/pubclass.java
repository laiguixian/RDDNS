...
public class pubclass {
	...
	public String getstringfromresource(String resourcepath){
		...
		try {
			...
			System.out.println("获取的内容："+resultstr);
		}catch (Exception e){
			System.out.println("获取的内容出错："+e.getMessage());
		}
		return resultstr;
	}
	...
	*/
	...
	public String getIpAddress(HttpServletRequest request) {
		...
		}System.out.println("初始ip="+ip);
		return ip;

	}

	//获取正确的ip
	public String getcreip(String inipstr) {
	...
	//从文章识别码里解析出文章主键
	public String getidfromarticlereg(String instr) {
		String returnvalue="";
		returnvalue=instr.substring(3);
		returnvalue=returnvalue.substring(0, returnvalue.length()-5);
		try {
			int tryint=Integer.parseInt(returnvalue);
		}catch(Exception ee) {
			System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"：文章识别码获取主键出错：识别码"+instr+"获取的主键："+returnvalue);
			returnvalue="0";
		}
	...
	//从用户识别码里解析出用户主键
	public String getidfromuserreg(String instr) {
		String returnvalue="";
		returnvalue=instr.substring(3);
		returnvalue=returnvalue.substring(0, returnvalue.length()-5);
		return returnvalue;
